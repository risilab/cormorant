...
class TestSO3Vec():
    ...
    def test_SO3Vec_init_arb_tau(self, batch, maxl, channels):
        ...
        tau_list = torch.randint(0, channels, [maxl+1])
    ...
    def test_SO3Vec_check_batch_fail(self, batch, channels):
        ...
        tau = torch.randint(0, channels, [maxl+1])
    ...
    def test_SO3Vec_add_list(self, batch, maxl, channels):
        ...
        scalar = [1 for _ in vec0]

        vec1 = scalar + vec0
        assert all(torch.allclose(1 + part0, part1) for part0, part1 in zip(vec0, vec1))

        vec1 = vec0 + scalar
        assert all(torch.allclose(part0 + 1, part1) for part0, part1 in zip(vec0, vec1))
