...
import tarfile, os, logging, numpy as np
...
def process_xyz_files(data, process_file_fn, file_ext=None, file_idx_list=None, stack=True):
    ...
    :data: Complete path to datafiles. Files must be in a director, tarball, or zip archive.
    :process_file_fn: Function to process files. Can be defined externally.
    ...
    :file_ext: Optionally add a file extension if multiple types of files exist.
    :file_idx_list: Optionally add a file filter to check a file index is in a
...
def process_xyz_md17(datafile):
    """
    Read xyz file and return a molecular dict with number of atoms, energy, forces, coordinates and atom-type
    :param lines of MD17 xyz-file:
    :return xyz data:
    """
